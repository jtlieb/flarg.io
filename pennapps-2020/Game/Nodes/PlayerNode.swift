...
func newPlayerNode(team: Team) -> SCNNode {
    ...
    body.geometry?.firstMaterial?.diffuse.contents = team == .red ? UIColor.systemPink : UIColor.systemIndigo
    body.position = SCNVector3(0, -1, 0)
    
    head.geometry = SCNSphere(radius: 0.2)
    head.geometry?.firstMaterial?.diffuse.contents = team == .red ? UIColor.systemPink : UIColor.systemIndigo
